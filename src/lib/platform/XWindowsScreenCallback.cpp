...
//
// XWindowsScreenCallback
//

// NOTE -- the X display is shared among several objects but is owned
// by the XWindowsScreenCallback.  Xlib is not reentrant so we must ensure
...
XWindowsScreenCallback*		XWindowsScreenCallback::s_screen = NULL;

XWindowsScreenCallback::XWindowsScreenCallback(
	...
	PlatformScreen(events)
{
	...
	XSetIOErrorHandler(&XWindowsScreenCallback::ioErrorHandler);
	...
	m_events->adoptHandler(Event::kSystem, m_events->getSystemTarget(),
							new TMethodEventJob<XWindowsScreenCallback>(this,
								&XWindowsScreenCallback::handleSystemEvent));
...
void
XWindowsScreenCallback::disable()
{
...
bool
XWindowsScreenCallback::setClipboard(ClipboardID id, const IClipboard* clipboard)
{
...
void
XWindowsScreenCallback::checkClipboards()
{
	// do nothing, we're always up to date
}

void
XWindowsScreenCallback::openScreensaver(bool notify)
{
...
void
XWindowsScreenCallback::closeScreensaver()
{
	if (!m_screensaverNotify) {
...
void
XWindowsScreenCallback::screensaver(bool activate)
{
	if (activate) {
...
void
XWindowsScreenCallback::resetOptions()
{
	m_xtestIsXineramaUnaware = true;
	m_preserveFocus = false;
}
...
void
XWindowsScreenCallback::setOptions(const OptionsList& options)
{
...
void
XWindowsScreenCallback::setSequenceNumber(UInt32 seqNum)
{
	m_sequenceNumber = seqNum;
}

bool
XWindowsScreenCallback::isPrimary() const
{
	return m_isPrimary;
}

void*
XWindowsScreenCallback::getEventTarget() const
{
	return const_cast<XWindowsScreenCallback*>(this);
}

bool
XWindowsScreenCallback::getClipboard(ClipboardID id, IClipboard* clipboard) const
{
...
void
XWindowsScreenCallback::getShape(SInt32& x, SInt32& y, SInt32& w, SInt32& h) const
{
	x = m_x;
	y = m_y;
	w = m_w;
...
void
XWindowsScreenCallback::getCursorPos(SInt32& x, SInt32& y) const
{
	Window root, window;
...
void
XWindowsScreenCallback::reconfigure(UInt32)
{
	// do nothing
}

void
XWindowsScreenCallback::warpCursor(SInt32 x, SInt32 y)
{
	// warp mouse
...
UInt32
XWindowsScreenCallback::registerHotKey(KeyID key, KeyModifierMask mask)
{
...
void
XWindowsScreenCallback::unregisterHotKey(UInt32 id)
{
	// look up hotkey
...
void
XWindowsScreenCallback::fakeInputBegin()
{
	// FIXME -- not implemented
}

void
XWindowsScreenCallback::fakeInputEnd()
{
	// FIXME -- not implemented
}

SInt32
XWindowsScreenCallback::getJumpZoneSize() const
{
	return 1;
}

bool
XWindowsScreenCallback::isAnyMouseButtonDown(UInt32& buttonID) const
{
...
void
XWindowsScreenCallback::getCursorCenter(SInt32& x, SInt32& y) const
{
	x = m_xCenter;
	y = m_yCenter;
}
...
void
XWindowsScreenCallback::fakeMouseButton(ButtonID button, bool press)
{
...
void
XWindowsScreenCallback::fakeMouseMove(SInt32 x, SInt32 y)
{
...
void
XWindowsScreenCallback::fakeMouseRelativeMove(SInt32 dx, SInt32 dy) const
{
...
void
XWindowsScreenCallback::fakeMouseWheel(SInt32, SInt32 yDelta) const
{
...
Display*
XWindowsScreenCallback::openDisplay(const char* displayName)
{
	// get the DISPLAY
	if (displayName == NULL) {
...
void
XWindowsScreenCallback::saveShape()
{
...
Window
XWindowsScreenCallback::openWindow() const
{
...
void
XWindowsScreenCallback::sendEvent(Event::Type type, void* data)
{
	m_events->addEvent(Event(type, getEventTarget(), data));
}

void
XWindowsScreenCallback::sendClipboardEvent(Event::Type type, ClipboardID id)
{
...
IKeyState*
XWindowsScreenCallback::getKeyState() const
{
	return m_keyState;
}

Bool
XWindowsScreenCallback::findKeyEvent(Display*, XEvent* xevent, XPointer arg)
{
...
void
XWindowsScreenCallback::handleSystemEvent(const Event& event, void*)
{
	...
	if (m_isPrimary) {
		if (xevent->type == KeyRelease) {
			...
			isRepeat = (XCheckIfEvent(m_display, &xevent2,
							&XWindowsScreenCallback::findKeyEvent,
...
void
XWindowsScreenCallback::onKeyPress(XKeyEvent& xkey)
{
	...
	if (key != kKeyNone) {
      ...
      KeyState ks;
      ks.keyCode = xkey.keycode;
      ks.isDown = true;
      fakeKeyStateQueue.push(ks);
...
void
XWindowsScreenCallback::onKeyRelease(XKeyEvent& xkey, bool isRepeat)
{
	const KeyModifierMask mask = m_keyState->mapModifiersFromX(xkey.state);
	KeyID key                  = mapKeyFromX(&xkey);
	if (key != kKeyNone) {
		...
		if (!isRepeat) {
         ...
         // BB - Do not bother logging the fake repeat here since
         //      press event is discarded
		}
	}
}

bool
XWindowsScreenCallback::onHotKey(XKeyEvent& xkey, bool isRepeat)
{
	// find the hot key id
...
void
XWindowsScreenCallback::onMousePress(const XButtonEvent& xbutton)
{
...
void
XWindowsScreenCallback::onMouseRelease(const XButtonEvent& xbutton)
{
...
void
XWindowsScreenCallback::onMouseMove(const XMotionEvent& xmotion)
{
...
Cursor
XWindowsScreenCallback::createBlankCursor() const
{
...
ClipboardID
XWindowsScreenCallback::getClipboardID(Atom selection) const
{
	for (ClipboardID id = 0; id < kClipboardEnd; ++id) {
		if (m_clipboard[id] != NULL &&
...
void
XWindowsScreenCallback::processClipboardRequest(Window requestor,
...
void
XWindowsScreenCallback::destroyClipboardRequest(Window requestor)
{
	// check every clipboard until one returns success
	for (ClipboardID id = 0; id < kClipboardEnd; ++id) {
		if (m_clipboard[id] != NULL &&
...
void
XWindowsScreenCallback::onError()
{
...
int
XWindowsScreenCallback::ioErrorHandler(Display*)
{
...
void
XWindowsScreenCallback::selectEvents(Window w) const
{
...
void
XWindowsScreenCallback::doSelectEvents(Window w) const
{
...
KeyID
XWindowsScreenCallback::mapKeyFromX(XKeyEvent* event) const
{
	// convert to a keysym
	KeySym keysym;
...
ButtonID
XWindowsScreenCallback::mapButtonFromX(const XButtonEvent* event) const
{
...
unsigned int
XWindowsScreenCallback::mapButtonToX(ButtonID id) const
{
...
void
XWindowsScreenCallback::warpCursorNoFlush(SInt32 x, SInt32 y)
{
...
void
XWindowsScreenCallback::updateButtons()
{
...
bool
XWindowsScreenCallback::grabMouseAndKeyboard()
{
...
void
XWindowsScreenCallback::refreshKeyboard(XEvent* event)
{
...
//
// XWindowsScreenCallback::HotKeyItem
//

XWindowsScreenCallback::HotKeyItem::HotKeyItem(int keycode, unsigned int mask) :
	m_keycode(keycode),
	m_mask(mask)
{
	// do nothing
...
bool
XWindowsScreenCallback::HotKeyItem::operator<(const HotKeyItem& x) const
{
...
bool
XWindowsScreenCallback::detectXI2()
{
	int event, error;
...
#ifdef HAVE_XI2
void
XWindowsScreenCallback::selectXIRawMotion()
{
	XIEventMask mask;
