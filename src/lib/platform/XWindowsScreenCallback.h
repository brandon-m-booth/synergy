...
class XWindowsScreenCallback : public PlatformScreen {
public:
	XWindowsScreenCallback(const char* displayName, bool isPrimary,
		bool disableXInitThreads, int mouseScrollDelta,
		IEventQueue* events);
	virtual ~XWindowsScreenCallback();
...
private:
	...
	// ioErrorHandler().
	static XWindowsScreenCallback*	s_screen;
};
